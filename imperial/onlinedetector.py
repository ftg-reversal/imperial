...
#
#
from imperial.featureextractor import FeatureExtractor
import cv2
import scipy
...
class RFDetector(Detector):
    ...
    def classify_proba(self, raw_feature):
        if self.classifier is None:
            raise Exception("should be trained before classify")
        probas = self.classifier.classify_proba(self._extract_raw_feature(raw_feature).T)
        return probas[0]
...
class BagofFeaturesDetector(OnlineDetector):
    ...
    def update(self, raw_feature, classid):
        ...
        if self.teacher_vectors is None:
            ...
            features = self._extract_raw_feature(raw_feature)
            if features != []:
                self.teacher_vectors = scipy.vstack((self.teacher_vectors, features))
