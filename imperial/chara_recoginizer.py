...
from imperial.icon_recoginizer import IconRecoginizer
...
class CharaRecoginizer(IconRecoginizer):
    ...
    # Define chara classification specific features.
    ...
    def model_filename(self, player, chara=None):
        if chara is None:
            return 'data/' + player + '/charas.knn.data'
        else:
            return 'data/' + player + '/charas_' + chara + '.knn.data'

    def load_model_from_file(self, player, chara=None, model_file=None):
        if model_file is None:
            model_file = self.model_filename(player, chara)

        super(CharaRecoginizer, self).load_model_from_file(model_file)

    def save_model_to_file(self, player, chara=None, model_file=None):
        if model_file is None:
            model_file = self.model_filename(player, chara)

        super(CharaRecoginizer, self).save_model_to_file(model_file)
    ...
    def __init__(self, model_file=None):
        ...
        super(CharaRecoginizer, self).__init__(k=5)
